...
### <span id="24">24.什么是设计模式，描述几个常用的设计模式？ </span>
...
我个人在工作当中使用过策略模式、单例模式、代理模式等设计模式。


##### 现在就以我工作中遇到的使用策略模式的场景为例：

> 需求：系统中合同号原始功能为自动递增，新需求要求支持客户自定义合同编号和作废合同编号的释放回收。

> 在不使用设计模式的情况下实现新需求，就需要写不同的方法来处理不同的情况。另外还需要考虑未来的扩展性，例如合同号的来源又有别的途径时，如果最少最快最好的解决。

> 所以经过分析后，我决定使用策略模式进行代码的重构。


##### 实现策略模式

策略模式可以将定义、创建、使用三部分进行解耦。

> 定义:一个策略接口和一组实现这个接口的策略类
 
策略接口：

![](https://github.com/binbinshan/Review-Up/blob/master/images/Java/16268589226823.jpg)

三个实现策略类：

![](https://github.com/binbinshan/Review-Up/blob/master/images/Java/16268590273177.jpg) 

![](https://github.com/binbinshan/Review-Up/blob/master/images/Java/16268590897013.jpg)

![](https://github.com/binbinshan/Review-Up/blob/master/images/Java/16268591429535.jpg)


> 创建:通过类型（type）来判断创建哪个策略来使用。需要对客户端代码屏蔽创建细节。把根据type创建策略的逻辑抽离出来。
> 
> 使用:客户端代码一般如何确定使用哪个策略呢？最常见的是运行时动态确定使用哪种策略，这也是策略模式最典型的应用场景。

为什么我把上面两步放在一起说呢，这是因为@Autowired提供了强大的功能。在注入集合的时候可以将集合类的泛型类填入集合作为初始值。这样我们就能根据bean的名字和功能枚举的对应关系，调用指定的策略进行处理。如下图

![](https://github.com/binbinshan/Review-Up/blob/master/images/Java/16268606820036.jpg)

当这个策略处理类加载到IOC容器中时，strategyMap中就会有三个实现了策略接口的策略类，
其中key是策略类的bean name，value则是bean实例。

在saveReserveContractNbr方法中，因为我设计时，策略类的bean name 和 执行哪个操作的枚举 是一致的，所以根据这个条件，可以去调用不同的bean实例(策略类)执行逻辑。

这样当我们又有新的功能分支需要处理，只需要添加枚举和对应的策略类即可。
